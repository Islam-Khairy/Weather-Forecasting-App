...
export const fetchWeatherData = async (dispatch, input) => {
  ...
  try {
    ...
    if (typeof input === 'string') {
      url = `https://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=${input}&days=7&aqi=no&alerts=no&units=metric&lang=ar`;
    } else if (typeof input === 'object') {
      const { latitude, longitude } = input;
      url = `https://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=${latitude},${longitude}&days=7&aqi=no&alerts=no&units=metric&lang=ar`;
    }
